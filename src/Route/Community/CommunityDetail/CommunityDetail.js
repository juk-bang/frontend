...
const CommunityDetail = ({
  ...
  const handleClickForwardPost = e => {
    document.location.href = `/community/detail/${univid}/${Post.previd}`;
  };
  //다음글 클릭
  const handleClickNextPost = e => {
    document.location.href = `/community/detail/${univid}/${Post.nextid}`;
  };
  //삭제 클릭
  ...
  const loaddata = async () => {
    //api로 부터 글 정보 불러오기
    let serverPostList = await getCommunityPost(univid, postid);
