...
import { getCommunityPost, postCommunityPut } from "Components/Api";
//Style
...
const Modify = ({
  ...
  const getCommunityPosts = async (univid, postid) => {
    //api로 부터 내용 불러오기
    const post = await getCommunityPost(univid, postid);
  ...
  //처음 render할 때 콘텐츠 로딩
  useEffect(() => {
    getCommunityPosts(univid, postid);
  }, []);
